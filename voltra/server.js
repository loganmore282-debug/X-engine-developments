...
app.use(async (req, res, next) => {
  ...
  if (await isMaintenanceOn())
    return res.status(503).json({ status: 'error', maintenance: true,
      message: 'Voltra is under maintenance. Please check back shortly. ◈' });
  next();
});

// ── HELPERS ──
function fmtUGX(n)   { return 'UGX ' + Number(n || 0).toLocaleString('en-UG'); }
function hashPin(p)  { return crypto.createHash('sha256').update(String(p) + 'voltra_salt_2026').digest('hex'); }
...
function phoneToEmail(phone) { return String(phone).replace(/\D/g,'') + '@voltra-app.com'; }
...
app.post('/sms/incoming', async (req, res) => {
  // Voltra SMS Forwarder app sends: { message, sender, secret }
...
app.post('/deposit/initiate', async (req, res) => {
  ...
  try {
    ...
    const receivingName = isMTN
      ? (settings.mtnReceivingName || 'VOLTRA INVESTMENTS')
      : (settings.airtelReceivingName || 'VOLTRA INVESTMENTS');
...
app.post('/auth/login', async (req, res) => {
  ...
  const email = phone.replace(/\D/g, '') + '@voltra-app.com';
  try {
  ...
  try {
    ...
    return res.json({
      ...
      settings: {
        ...
        aboutImage1:         s.aboutImage1          || '',
        aboutImage2:         s.aboutImage2          || '',
        aboutImage3:         s.aboutImage3          || '',
        aboutTitle1:         s.aboutTitle1          || '',
        aboutBody1:          s.aboutBody1           || '',
        aboutTitle2:         s.aboutTitle2          || '',
