...
(async () => {
  ...
  console.log('\n== Step 1: buy Hershey\'s at today\'s rate (30,000 -> 750,000, the "before the change" purchase) ==');
  ...
  check('locked in at 750,000 total, 30,000 price', oldInv?.expectedReturn === 750000 && oldInv?.amount === 30000, oldInv);
  const oldDaily = oldInv.dailyPayout;
  check('locked-in daily figure is 4,167', oldDaily === 4167, oldInv);
  ...
  check('expectedReturn is still 750,000 (not the new 999,999)', untouchedInv.expectedReturn === 750000, untouchedInv);
  check('cycle is still 180 (not the new 90)', untouchedInv.cycle === 180, untouchedInv);
  check('dailyPayout is still 4,167 (not re-derived from the new global returnMultiple/cycleDays)', untouchedInv.dailyPayout === 4167, untouchedInv);
  ...
  check('3 elapsed days pays exactly 3 x 4,167 = 12,501, not any new-rate figure',
    userDoc(A).walletBalance === balBefore + expected,
    { balance: userDoc(A).walletBalance, expected: balBefore + expected });

  console.log('\n-- Maturity on the old investment still pays out exactly the original 750,000 total --');
  ...
  check('matures at exactly 750,000 paid out, never the new total-payout figure', maturedOld.status === 'matured' && maturedOld.paidOut === 750000, maturedOld);
  ...
  check('new purchase reflects the NEW total payout (999,999), not the old 750,000', newInv?.expectedReturn === 999999, newInv);
