/* CHOCOMCC ATTACH-REFERRER-AFTER-THE-FACT
...
(async () => {
  ...
  userDoc(CHILD).walletBalance = 1000000; // fund them so they can buy a chocolate below
  ...
  r = await call('POST', '/invest/create', { token: 'uid:' + CHILD, body: { tierKey: 'hersheys' } });
  ...
  r = await call('POST', '/invest/create', { token: 'uid:' + LATE, body: { tierKey: 'hersheys' } });
  check('LATE buys their first chocolate with no referrer at all -- no commission possible yet', r.body?.status === 'success', r.body);
