/* CHOCOMCC PURCHASE RACE-CONDITION AUDIT
...
(async () => {
  ...
  const [r1, r2] = await Promise.all([
    call('POST', '/invest/create', { token: 'uid:' + A, body: { tierKey: 'hersheys' } }),
    call('POST', '/invest/create', { token: 'uid:' + A, body: { tierKey: 'hersheys' } }),
  ]);
  ...
  const burst = await Promise.all(Array.from({ length: 10 }, () =>
    call('POST', '/invest/create', { token: 'uid:' + B, body: { tierKey: 'hersheys' } })
  ));
  ...
  const rC = await call('POST', '/invest/create', { token: 'uid:' + C, body: { tierKey: 'hersheys' } });
