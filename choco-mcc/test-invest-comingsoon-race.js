/* CHOCOMCC "COMING SOON" PURCHASE-TIME DOUBLE-CHECK
...
(async () => {
  ...
  await ownerCall('/admin/products/save', { products: [{ key: 'rondnoir', name: 'Ferrero Rondnoir', price: 1000000, cycle: 180, expectedReturn: 20000000, comingSoon: true }] });
  const balBefore = userDoc(U).walletBalance;
  let r = await call('POST', '/invest/create', { token: 'uid:' + U, body: { tierKey: 'rondnoir' } });
  ...
  check('no investment record created', [...collMap('investments').values()].filter(i => i.userId === U && i.tierKey === 'rondnoir').length === 0);

  console.log('\n== A genuinely active product still purchases normally (no false positive from the extra check) ==');
  await ownerCall('/admin/products/save', { products: [{ key: 'rocher', name: 'Ferrero Rocher', price: 2000000, cycle: 180, expectedReturn: 40000000, active: true, comingSoon: false }] });
  const balBefore2 = userDoc(U).walletBalance;
  r = await call('POST', '/invest/create', { token: 'uid:' + U, body: { tierKey: 'rocher' } });
  ...
  const rocherInv = [...collMap('investments').values()].find(i => i.userId === U && i.tierKey === 'rocher');
  ...
  await ownerCall('/admin/products/save', { products: [{ key: 'rocher', name: 'Ferrero Rocher', price: 2000000, cycle: 180, expectedReturn: 40000000, comingSoon: true }] });
  const rocherInvAfterPause = [...collMap('investments').values()].find(i => i.userId === U && i.tierKey === 'rocher');
  ...
  check('still only ONE rocher investment exists for this user (no duplicate slipped through)', [...collMap('investments').values()].filter(i => i.userId === U && i.tierKey === 'rocher').length === 1);
