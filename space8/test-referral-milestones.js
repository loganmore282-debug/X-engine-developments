...
(async () => {
  ...
  check('claim succeeds, pays UGX 7,500', r.body?.status === 'success' && r.body?.amount === 7500, r.body);
  check('wallet credited +7,500 over starting balance', userDoc(REF).walletBalance === START_BAL + 7500, userDoc(REF).walletBalance);
  r = await call('POST', '/team/milestone/claim', { token: 'uid:' + REF, body: { target: 5 } });
  check('re-claim rejected (already claimed)', r.code === 400 && /already/i.test(r.body?.message || ''), r.body);
  check('wallet unchanged after re-claim attempt', userDoc(REF).walletBalance === START_BAL + 7500, userDoc(REF).walletBalance);
  ...
  check('wallet unchanged', userDoc(REF).walletBalance === START_BAL + 7500, userDoc(REF).walletBalance);
  ...
  check('wallet now +7,500+12,500=+20,000 over starting balance', userDoc(REF).walletBalance === START_BAL + 20000, userDoc(REF).walletBalance);
