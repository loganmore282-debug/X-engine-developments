...
const INTENTS = [
  { id: 'security_secret', priority: 5,
    ...
    reply: (ctx) => /forgot/i.test(ctx.message)
      ? '🔐 Passwords are handled by Firebase Authentication and are never readable by staff or by me. The sign-in screen has no self-service reset — contact Customer Service with your registered phone number and they can reset it for you. Your withdrawal PIN is separate — see Account → Change PIN.'
      : "🔒 I can't see or share your PIN or password — they're never stored anywhere in a readable form, only as a one-way hash, so there's genuinely nothing to hand over even if I wanted to. You can change your PIN anytime from Account → Change PIN." },
  ...
  { id: 'capability', priority: 1,
    ...
    reply: () => "🛰️ I can help with deposits, withdrawals, investing (including specific plan details), referrals and commissions, check-ins, your live balance, the withdrawal PIN, and general questions about how Space8 works. Ask away!" },
  ...
  { id: 'withdraw', priority: 2, kw: { withdraw: 3, withdrawal: 3, cashout: 3, payout: 2, payouts: 2 },
    ...
    reply: (ctx) => {
      ...
      return `💸 Bind a bank account first (Account → Bind Bank Account) if you haven't already, then tap Withdraw on Home. Minimum withdrawal is ${fmt(m)}, and a ${f}% fee applies. You'll need your withdrawal PIN to confirm — that's what keeps a withdrawal safe even if your phone isn't.${extra}`;
    } },
  ...
  { id: 'pin', priority: 3, kw: { pin: 3, pincode: 3 },
    ...
    reply: () => "🔒 Your withdrawal PIN was set when you registered. To change it, go to Account → Change PIN — you'll need your current one first. It's required to bind a bank account and for every withdrawal, so even someone with your unlocked phone can't move money out without it." },
  ...
  { id: 'banned', priority: 2, kw: {banned:3,suspended:3,restricted:3,disabled:2},
    ...
    reply: (ctx) => {
      ...
      const contact = s.supportTelegram || s.whatsappContact ? ` Reach ${s.supportTelegram ? 'Telegram ' + s.supportTelegram : 'WhatsApp ' + s.whatsappContact} to ask about it.` : ' Reach Customer Service from the Account tab to ask about it.';
  ...
  { id: 'support', priority: 2, kw: { support: 3, contact: 2, human: 2, agent: 2, complaint: 2, problem: 1, issue: 1 },
    ...
    reply: (ctx) => {
      ...
      return parts.length ? `🎧 Reach our team directly — ${parts.join(', ')}.` : '🎧 Reach our customer service team from Account → Customer Service.';
    } },
  ...
  { id: 'giftcode', priority: 2,
    ...
    reply: () => `🎁 If you have a gift/redeem code, tap the gift icon on Home and enter it there — a valid code credits your wallet immediately. Codes are single-use per account.` },
  ...
  { id: 'security_general', priority: 1,
    ...
    reply: () => `🔒 Your login is protected by Firebase Authentication, and every sensitive action (binding a bank account, withdrawing) is gated behind your withdrawal PIN on top of that. Your PIN itself is never stored in a readable form — only as a one-way hash.` },
  ...
  { id: 'about', priority: 1, phrase: [/is (this|it) (genuine|real|legit|trustworthy)/i, /what is this (app|platform|site)/i], kw: { space8: 2, platform: 2, legit: 2, legitimate: 2, safe: 2, scam: 2, trust: 2 },
    reply: () => pick([
      '🛰️ Space8 is a mobile-money investment platform — you invest in a satellite-themed plan and it pays out a fixed, pre-stated return through automatic daily cashback across its cycle. Deposits run through mobile money; withdrawals go to mobile money or a bound bank account, and every transaction (deposit, investment, cashback, withdrawal) is logged on your Account history, nothing happens silently.',
  ...
  { id: 'install_app', priority: 3,
    ...
    reply: () => "📲 Space8 works as an installable app right from your browser — no app store needed. Open Account and tap Get App (it's just above Exit). If your browser doesn't support one-tap install, use its menu and choose \"Add to Home Screen\" instead." },
  ...
  { id: 'multi_withdrawal_accounts', priority: 7,
    ...
    reply: () => '📱 Yes — you can bind more than one bank account. Add each one from Account → Bind Bank Account, then pick which one to use each time you withdraw by tapping the account row on the Withdraw screen.' },
  ...
  { id:'max_withdraw_limit',priority:3, kw: {cap:3},phrase:[/how much can i withdraw per day/i, /maximum withdraw/i,/withdraw(al)? limit/i,/how many (times|withdrawals) .*(day|daily)/i],reply:c=>c.settings.maxWithdrawalsPerDay?`💸 You can make up to ${c.settings.maxWithdrawalsPerDay} withdrawal${c.settings.maxWithdrawalsPerDay===1?'':'s'} per day. Each one is also capped at ${fmt(999999999)} as a system safety limit, on top of your available wallet balance.`:`💸 There's no fixed daily count limit, but each withdrawal is capped at ${fmt(999999999)} as a system safety limit, on top of your available wallet balance.` },
  ...
  { id:'account_deletion',priority:6, kw: {deactivate:3},phrase:[/delete (my )?profile/i, /i want to (quit|leave)/i, /(delete|close|remove|deactivate) (my )?account/i,/how (do i|to) deactivate/i],reply:()=> '👤 Account deletion isn\'t self-service in the app right now — contact Customer Service with your registered phone number and they\'ll guide you through it.' },
  ...
  { id:'support_hours_check',priority:3,phrase:[/are you open/i, /working hours/i, /support (time|hours)/i, /support (hours|available|24)/i,/when (is|are) support (open|available)/i],reply:c=>c.settings.supportHours?`🎧 Customer Service hours: ${c.settings.supportHours}.`:'🎧 Check Account → Customer Service for current hours — reach out any time and the team will respond as soon as they can.' },
  ...
  { id:'tax_question',priority:2,phrase:[/tax(es)?/i,/does space8 (pay|handle|deduct) tax/i],reply:()=> '💵 Space8 doesn\'t withhold or file taxes on your behalf — any personal tax obligation on your earnings is yours to handle. Keep your transaction history (Account → Deposit Records/Withdrawal Records) for your own records.' },
  { id:'company_info',priority:6,phrase:[/company details/i, /^management/i, /who (owns|runs|is behind)/i,/who is the (ceo|founder|owner)/i],reply:()=> '🛰️ I don\'t have details on ownership or company structure to share — that\'s outside what I can answer. Reach Customer Service if you need that kind of information.' },
  ...
  // ── DEPOSITS, deeper ──
  { id:'deposit_max',priority:3,phrase:[/max(imum)? i can deposit/i, /maximum deposit/i,/deposit limit/i,/most i can deposit/i],reply:()=> `💰 The minimum is enforced, and a single deposit is also capped at ${fmt(999999999)} as a system safety limit. You can deposit as many separate times as you like within that cap.` },
  ...
  { id:'deposit_reference',priority:3,phrase:[/ref(erence)? number/i, /where is the reference/i, /transaction reference/i, /deposit reference/i,/reference (number|id|code)/i,/where.{0,15}(find|get).{0,15}reference/i],reply:()=> '🧾 Every deposit and withdrawal gets a unique reference (starting with "B") shown on its row under Account → Deposit Records / Withdrawal Records and in Records. Quote that reference whenever you contact Customer Service about a specific transaction.' },
  { id:'deposit_not_in_records',priority:6,phrase:[/deposit not listed/i, /not in my history/i, /deposit.{0,25}not (in|showing in) (records|history)/i,/(records|history).{0,20}missing.{0,15}deposit/i,/deposit.{0,20}missing (from )?(records|history)/i],reply:()=> '🧾 A deposit only appears in Records once it has actually been confirmed. If it succeeded but is missing there, pull to refresh or fully close and reopen the app first — then contact Customer Service with the reference if it still is not listed.' },
  ...
  { id:'withdraw_rejected',priority:6,phrase:[/payout declined/i, /(withdraw\w*|cash ?out).{0,20}(rejected|declined|refused|denied|failed)/i],reply:()=> '💸 A declined withdrawal returns the money to your wallet — you do not lose it. The usual causes are a name/number mismatch on the payout account or a provider-side failure. Check the reason shown on the row under Account → Withdrawal Records, correct the account details, and request again.' },
  { id:'withdraw_weekend',priority:2,phrase:[/weekend payout/i, /withdraw.{0,20}(weekend|sunday|saturday|holiday|night)/i],reply:(c)=>{
    const s = c.settings;
    if (!s.withdrawHoursEnabled) return '⏱️ Withdrawals are processed every day, including weekends and at night. Timing depends on review and mobile-money network conditions rather than office hours.';
    const h12 = h => { const ap = h < 12 ? 'AM' : 'PM'; let hh = h % 12; if (hh === 0) hh = 12; return hh + ':00 ' + ap; };
    return `⏱️ Withdrawal REQUESTS can currently only be submitted between ${h12(s.withdrawHoursStart)} and ${h12(s.withdrawHoursEnd)} East Africa Time (weekends included) — that window is admin-set and can change. Once a request is in, it's processed the same regardless of day or night.`;
  } },
  { id:'withdraw_history_where',priority:6,phrase:[/(past|see my) withdrawals/i, /where.{0,25}(see|check|find).{0,20}(withdraw\w*|cash ?out)/i,/withdrawal history/i],reply:()=> '🧾 Account → Withdrawal Records lists every request with its amount, date, reference and current status. Records (the doc icon on Home) shows them alongside every other transaction type.' },
  { id:'withdraw_to_bank',priority:6,phrase:[/send to my bank/i, /bank payout/i, /withdraw.{0,20}bank/i,/bank (transfer|account).{0,20}withdraw/i,/^bank transfer withdrawal/i],reply:()=> '🏦 Yes — bind your bank account under Account → Bind Bank Account, same as mobile money: pick your bank from the network list, enter your account holder name and account number, and confirm with your withdrawal PIN.' },
  ...
  { id:'forgot_pin',priority:7,phrase:[/pin forgotten/i, /recover (my )?pin/i, /(dont|do not|don.t) remember my pin/i, /forgot.{0,15}(my )?(withdrawal |payout |security )?pin/i,/(lost|cannot remember|can.?t remember).{0,15}pin/i,/reset.{0,15}pin/i],reply:()=> '🔒 A forgotten PIN cannot be recovered or read by anyone — it is stored only as a one-way hash. If you still know your current PIN you can change it under Account → Change PIN. If you genuinely cannot remember it, contact Customer Service from the Account tab; only they can arrange a reset, and they will never ask you to send them a PIN.' },
  { id:'change_pin',priority:7,phrase:[/update (my )?pin/i, /change.{0,15}(my )?(withdrawal |payout |security )?pin/i,/new pin/i],reply:()=> '🔒 Account → Change PIN. You will need your current PIN to set a new one. A new PIN cannot be four identical digits (1111, 0000 and so on) — pick four digits that are not all the same.' },
  ...
  { id:'change_password',priority:7,phrase:[/reset (my )?password/i, /change.{0,15}(my )?password/i,/(new|different) password/i],reply:()=> "🔐 If you're already signed in and still know your current password: Account → Password Management, enter the current one plus a new one (minimum 6 characters). If you've genuinely forgotten it and can't sign in at all, there's no self-service reset on the sign-in screen — contact Customer Service with your registered phone number and they can reset it for you. Your withdrawal PIN is completely separate and is changed under Account → Change PIN." },
  { id:'change_name',priority:6,phrase:[/(edit|change) my name/i, /name change/i, /(change|update).{0,20}(my )?(name|profile name|display name)/i],reply:()=> '👤 There is no editable display name — your account is identified by your registered phone number and your server-issued account ID. The holder name saved on a bank account is separate and can be corrected by deleting and re-adding that account.' },
  { id:'account_hacked',priority:7, kw: {compromised:3,hacked:3},phrase:[/hacked/i,/unauthori[sz]ed/i,/someone (else )?(is )?(using|accessed).{0,20}(my )?account/i,/someone.{0,20}(took|stole).{0,15}money/i],reply:()=> '🚨 Act immediately: change your password under Account → Password Management, then change your withdrawal PIN under Account → Change PIN, then check Account → Bind Bank Account and delete any payout account you do not recognise. Contact Customer Service straight away with your registered number. Never share your PIN or password with anyone, including anyone claiming to be staff.' },
  ...
  { id:'platform_closes',priority:3,phrase:[/if you shut down/i, /platform closure/i, /(what|happens) if.{0,25}(platform|space8|site|app).{0,20}(close|shut|stop|down)/i],reply:()=> '🛰️ I cannot speak to future business decisions. What I can tell you is that your balance, plans and full transaction history are held server-side and visible to you at all times under Records. For anything about platform guarantees, Account → Rules and Customer Service are the right places.' },
  { id:'regulated',priority:3, kw: {regulation:3,regulated:3},phrase:[/(regulated|licence|license|registered company|legal|bank of uganda)/i],reply:()=> '🛰️ I do not hold licensing or registration details. Account → Rules carries the platform\'s official statements, and Customer Service can answer questions that page does not cover.' },
...
const FALLBACKS = [
  "🤔 I can help with deposits, withdrawals, investing, referrals, check-ins, your balance, or the withdrawal PIN — try asking about one of those!",
  ...
  "Hmm, I can help with things like deposits, withdrawals, investing or referrals 🙂 — or reach Customer Service from the Account tab for anything else."
];
...
const DEEP = {
  ...
  pin: () => `🔒 In detail: your withdrawal PIN is a second, separate secret from your login password, and it exists specifically so that a stolen password alone can't move money. It's required to add or delete a bank account and to confirm every withdrawal. It's stored only as a one-way hash — nobody, including platform staff and this assistant, can read it back. Nobody legitimate will ever ask you for it: not customer service, not an admin, not anyone on Telegram or WhatsApp. Change it anytime under Account → Change PIN using your current PIN.`,
  ...
  giftcode: () => `🎁 In detail: a gift code is a promotional credit issued by the Space8 team, usually through the Telegram community or a campaign. You redeem it by tapping the gift icon on Home and typing it in there. Codes are case-sensitive — capitals and lowercase must match exactly as issued — and each one can normally be used once per account. A valid code credits your wallet immediately and shows up in your records as a gift-code entry.`,
  ...
  payout_account: () => `📱 In detail: a bank account is a mobile-money OR bank destination you bind in advance, under Account → Bind Bank Account. Pick MTN, Airtel, or any supported bank from the network list. You can bind several and choose which one receives each withdrawal by tapping the account row on the Withdraw screen. Binding requires the account holder's name, the correct network, and a valid account number (mobile-money number or bank account number); deleting one requires your withdrawal PIN. Withdrawals only ever go to an account bound here — never to a number typed fresh at withdrawal time — which is a deliberate safeguard against someone redirecting your money.`,
  ...
  banned: () => `🔒 In detail: account suspension is usually a manual action by the platform team, tied to a specific concern on that account. A small number of accounts are also auto-suspended by the system itself (for example, repeated deposit attempts that never actually clear). Either way this assistant has no visibility into individual cases. While suspended, money actions are blocked. The only route is Customer Service: contact them with your registered phone number and ask what triggered it and what's needed to resolve it.`,
  ...
  min_withdraw_specific: (c) => `💸 In detail: the ${fmt(c.settings.minWithdraw)} minimum exists for the same reason as the deposit floor — each payout carries a fixed cost to send. The ${c.settings.withdrawFeePct}% fee applies on top of that minimum, so the smallest withdrawal nets you ${fmt(c.settings.minWithdraw - Math.round(c.settings.minWithdraw * c.settings.withdrawFeePct / 100))}. Beyond your available wallet balance, a single withdrawal is also capped at ${fmt(999999999)} as a system safety limit.`,
