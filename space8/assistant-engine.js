...
const INTENTS = [
  { id: 'security_secret', priority: 5,
    ...
    reply: (ctx) => /forgot/i.test(ctx.message)
      ? '🔐 Passwords are handled by Firebase Authentication and are never readable by staff or by me. Use the password-reset option on the sign-in screen. Your withdrawal PIN is separate — see Account → Security PIN.'
  ...
  { id:'min_deposit_specific',priority:3,phrase:[/min(imum)?( amount to)? deposit/i, /smallest deposit/i, /minimum deposit/i,/least (i|you) can deposit/i,/lowest deposit/i],reply:c=>`💰 The minimum deposit is ${fmt(c.settings.minDeposit)}. There's no maximum — deposit as much as you're comfortable investing.` },
  ...
  { id:'data_privacy',priority:2,phrase:[/data protection/i, /sell my data/i, /personal information/i, /is my data (safe|private|secure)/i,/privacy/i,/what (data|information) do you (collect|store)/i],reply:()=> '🔒 Only what\'s needed to run your account — phone number, transaction history, and your hashed PIN (never readable, even by staff). See Account → Terms of Service for the full picture.' },
  ...
  { id:'my_plans_where',priority:3, kw: {portfolio:3},phrase:[/see my active plans/i, /where.{0,25}(see|check|find).{0,20}(my )?(plan|product|investment)/i,/my (plans|products|investments)/i],reply:()=> '🚀 Your active plans show on Home with a progress ring and day count, and the Products tab summarises how many you hold plus your cumulative earnings.' },
  { id:'plan_progress',priority:6,phrase:[/plan day count/i, /progress of my investment/i, /(track|check|see).{0,20}(progress|how far|day count)/i,/how many days (are )?(left|remaining|to go)/i,/how far (is|along).{0,15}(my )?plan/i],reply:()=> '🚀 Each active plan on Home shows its own day counter (day X of Y) and a progress ring, computed server-side from the exact purchase time — so it never drifts from what you are actually owed.' },
  ...
  // ── CONVERSATIONAL / META ──
  { id:'are_you_sure',priority:2,phrase:[/^(really|for sure|you (sure|certain))[\s?.!]*$/i, /are you (sure|certain|correct|right)/i,/is that (right|correct|true)/i],reply:()=> '✅ Every figure I quote is read live from the platform settings and your own account at the moment you ask, not from a stored script — so the numbers are current. For anything about policy or a specific transaction, Account → Terms of Service and Support are the authoritative sources.' },
...
const DEEP = {
  ...
  cumulative_earnings: (c) => `📊 In detail: Cumulative Earnings sums money your plans have actually paid out (daily cashback plus final settlement) plus any Task Center mission rewards you've claimed. It deliberately excludes referral commission, check-in bonuses, the welcome bonus and gift codes — those are real money in your wallet, they're just tracked separately so "what have my investments earned" stays a clean, honest number rather than being inflated by bonuses. Team → Total Commission covers the referral side; your wallet balance is everything combined.`,
