# Space8 (working in `space8/` for now) — Project Memory (read this first)
...
**Space8**, but the owner said in this session: **"it is going to be 'space8'"** — the
project is renaming. As of this file being written, the code, folder name (`space8/`),
and all internal strings still say "Space8" — **the rename to "space8" has NOT been done
yet**. Do not assume it's done. Ask the owner at the start of the next session whether:
(a) "space8" fully replaces "Space8" everywhere (folder name, brand strings, tier-key
prefixes, storage keys, etc.), or (b) "Space8" stays as an internal/product name and
...
3. **User-facing app (`user-src/`, `user/`) — CURRENTLY WRONG, NEEDS A FRESH BUILD.**
   ...
   rejected this hard: *"we are building space8, you are just respraying choco... l dont
   want to rebrand ChocoMCC, make space8 on its own, different design, architecture."*
...
1. Built a small from-scratch Space8 scaffold (own server.js/db.js/index.html/admin.html,
...
Static mockup, already published, still live: **`space8/design/visual-system-mockup.html`**
...
- Repo: `loganmore282-debug/x-engine-developments` (GitHub) — a multi-project repo; this
  project's code lives under `space8/`, sibling to `voltra/`, `choco-mcc/`, `nexus/`, and
  others. **Never edit `choco-mcc/` or `voltra/` from this project's sessions.**
- Branch: `claude/new-session-9z5u7r`.
- The owner wants **Codex working alongside Claude** on this project. `space8/CODEX.md`
  points Codex sessions at this file. `space8/AGENT_LOG.md` is the shared changelog —
...
- `render.yaml` — Render deploy config (`rootDir: space8`, `rootDir: space8/user`).
- `design/visual-system-mockup.html` — the approved-direction (not yet owner-confirmed
  final) design mockup described above.

## Rebrand mapping already applied (ChocoMCC → Space8 strings/colours)

`ChocoMCC`→`Space8`, `CHOCO_*`→`SPACE8_*`, `choco_*` storage keys→`space8_*`. Colour
...
- Embedded product/banner fallback photos (`SPACE8_IMAGES`/`SPACE8_BANNERS`) are still literal
...
- Whether "space8" replaces "Space8" everywhere — unconfirmed, ask first.
