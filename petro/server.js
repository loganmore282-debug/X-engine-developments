...
// The Home activity card is deliberately a read-only, global view of REAL
// completed ledger entries.  It never writes money, never invents activity,
// and exposes only a country prefix plus the final two phone digits.  One
// server-side snapshot is shared for a minute so every phone sees the same
// five events rather than a client-randomized feed.
const PUBLIC_ACTIVITY_TTL_MS = 60 * 1000;
const publicActivityCache = { at: 0, activities: [], pending: null };
function maskPublicActivityPhone(phone) {
  let digits = String(phone || '').replace(/\D/g, '');
  if (!digits) return '';
  // Petro's primary platform format is Uganda's 256 prefix.  Some existing
  // member records have the local 07xxxxxxxx form, so normalize that form
  // before masking; a full international number retains its own prefix.
  if (digits[0] === '0' || digits.length === 9) digits = '256' + digits.replace(/^0+/, '');
  const prefix = digits.length >= 3 ? digits.slice(0, 3) : '256';
  const suffix = digits.slice(-2).padStart(2, '0');
  return prefix + ' **** ' + suffix;
}
async function getPublicActivityFeed() {
  const now = Date.now();
  if (now - publicActivityCache.at < PUBLIC_ACTIVITY_TTL_MS) return publicActivityCache.activities;
  if (publicActivityCache.pending) return publicActivityCache.pending;
  const pending = (async () => {
    const snap = await db.collection('transactions').orderBy('createdAt', 'desc').limit(80).get();
    const candidates = snap.docs.map(doc => ({ id: doc.id, ...doc.data() })).filter(row => {
      const type = String(row.type || '').toLowerCase();
      const status = String(row.status || '').toLowerCase();
      return status === 'success' && (type === 'deposit' || type === 'withdraw') && Math.abs(Number(row.amount) || 0) > 0;
    }).slice(0, 24);
    const userIds = [...new Set(candidates.map(row => String(row.userId || '')).filter(Boolean))];
    ...
    for (const row of candidates) {
      ...
      const phone = maskPublicActivityPhone(user.phone || user.mobile || user.msisdn);
      if (!phone) continue;
      activities.push({
        phone,
        action: String(row.type).toLowerCase() === 'withdraw' ? 'withdraw' : 'deposit',
        amount: Math.abs(Number(row.amount) || 0),
      });
    }
    publicActivityCache.activities = activities;
    publicActivityCache.at = Date.now();
    return activities;
  })();
  publicActivityCache.pending = pending;
  try { return await pending; }
  finally {
    if (publicActivityCache.pending === pending) publicActivityCache.pending = null;
  }
}
app.get('/public/activity-feed', async (req, res) => {
  try {
    publicJson(req, res, { status: 'success', activities: await getPublicActivityFeed() }, 'no-store');
  } catch (e) {
    console.error('Public activity feed error:', e.message);
    res.status(500).json({ status: 'error', message: 'Activity is temporarily unavailable.' });
  }
});
