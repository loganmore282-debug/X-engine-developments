/* NOVERA MAINTENANCE-MODE, PRODUCT-FLAG, AND BAN-COVERAGE AUDIT
...
(async () => {
  ...
  await adminCall('/admin/products/save', { products: [{ key: 'singularity', name: 'Singularity', price: 4000000, cycle: 180, expectedReturn: 80000000, comingSoon: true }] });
  let r = await call('GET', '/public/products');
  const godiva = (r.body?.products || []).find(p => p.key === 'singularity');
  ...
  r = await call('POST', '/invest/create', { token: 'uid:' + D, body: { tierKey: 'singularity' } });
