  ...
  try {
    ...
    if (s && s.maintenanceMode) {
      return res.status(503).json({ status: 'error', code: 'MAINTENANCE',
        message: s.maintenanceMsg || 'Novera is under maintenance. Please check back shortly.' });
    }
  } catch (_) {}
  next();
});
...
// (novera/user/index.html). Admin panel overrides live in the
...
function phoneToEmail(phone) { return String(phone).replace(/\D/g, '').replace(/^0+/, '') + '@novera.com'; }
...
// NOVA_BANNERS object ships with a default for. An admin override is
...
app.post('/admin/deposit', async (req, res) => {
  ...
  try {
    ...
    await db.runTransaction(async t => {
      ...
      t.set(db.collection('transactions').doc(), {
        userId, type: 'admin_credit', description: note || 'Novera credit',
...
// Built from the live referredBy graph — Novera has no separate
...
app.get('/', (_req, res) => res.json({ status: 'ok', service: 'Novera backend' }));
...
connectMongo(MONGODB_URI)
  .then(() => {
    app.listen(PORT, () => console.log(`Novera backend listening on :${PORT}`));
