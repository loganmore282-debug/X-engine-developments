...
**Owner's explicit framing (2026-08-26): "we started building real frontend, that is
why we are building and editing."** This is not a disposable design exercise — the
`snow/design/mockup-src/*.html` screens (generated by `build.py`) are the actual
visual/structural foundation the real app will be built from, not throwaway
concepts. Given that, an explicit sequencing decision was made the same day: **finish
locking every screen's design first (via the fast generator+screenshot+Codex-critique
loop already in use), THEN convert the whole thing into the real single-page app in
one pass** — rather than converting screen-by-screen as each one gets approved, which
would mean rebuilding/rewiring already-converted screens every time a later screen's
critique round reveals a shared-component change (exactly what happened across Account
...
space8 used (see `space8/CLAUDE.md` for the pattern) — backend logic reused/adapted
from a proven base, admin panel reskinned not rebuilt, user-facing frontend genuinely
new. Do not assume space8's `server.js`/`db.js` can be copied verbatim — the product
ladder, rates, and nav structure above are all different and need to be reflected in
whatever backend gets built. The mockup screens' CSS tokens/component classes
(`--snow-*` variables, `.app-card`/`.primary-button`/`.account-*` etc. in `build.py`)
should carry over directly into the real `user-src/index.html`'s stylesheet rather
