...
ROUTES={
 ...
 "/bank/list":{"status":"success","accounts":[{"id":"w1","network":"MTN Mobile Money","phone":"0769968158","holder":"Mangalita Namugabwe"}]},
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        await page.evaluate("openWithdrawSheet()"); await page.wait_for_timeout(1400)
        info = await page.evaluate("""()=>{const chip=document.querySelector('.wallet-card .chip');
          const cs=chip?getComputedStyle(chip,'::before'):null;
          const icon=document.querySelector('.wallet-card .row1 svg');
          ...
          return {chip: chip?{w:chip.getBoundingClientRect().width,h:chip.getBoundingClientRect().height,
                  ...
                  receiveShown: hint?getComputedStyle(hint).display!=='none':null,
                  witAmt: document.querySelector('.wit-amt').getBoundingClientRect().height,
        ...
        ck(info["receiveShown"] is False, "resting screen shows Fee only, like the mockup")
        await page.screenshot(path=f"{OUT}/withdraw-card.png",full_page=True)
        # typing an amount should reveal the net figure
        await page.fill('#witAmount','20000'); await page.wait_for_timeout(400)
        await page.evaluate("syncWithdrawReceiveAmt()"); await page.wait_for_timeout(300)
        shown = await page.evaluate("()=>{const h=document.querySelector('#witReceiveHint');return {vis:getComputedStyle(h).display!=='none', txt:h.innerText.trim()};}")
        print("  after typing:", shown)
        ck(shown["vis"] and "17,000" in shown["txt"], "net appears once an amount is entered: "+shown["txt"])
        ck(not errs,"no page errors: "+str(errs))
        await b.close()
    print(("\n%d FAILED" % len(fails)) if fails else "\nwithdraw: all cases pass")
