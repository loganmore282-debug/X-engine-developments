...
async function ensureIndexes() {
  const specs = [
    ...
    // ── UNIQUENESS ENFORCED BY THE DATABASE, NOT ONLY BY APPLICATION CODE ──
    // Every value below is already generated by a "make a candidate, query to
    // see if it's taken, write it" loop in server.js
    // (generateUniqueReferralCode, generateUniqueGiftCode, uniqueRef,
    // nextSequentialPublicId). That check-then-write is not atomic: two
    // registrations landing in the same instant can both read "free" and both
    // write the same code. It has never needed a real transaction to fix,
    // because a unique index does the job -- Mongo refuses the second write
    // outright, on M0, Flex or dedicated alike, and unlike the in-process
    // Sets this codebase uses as write locks it keeps working if this server
    // is ever run as more than one instance.
    //
    // Why these matter, in order of what a collision would actually cost:
    //   referralCodeLower -- findUserByReferralCode() takes the FIRST match,
    //     so two users sharing a code sends one person's L1/L2/L3 commission
    //     to the other. Real money, silently misrouted, very hard to notice.
    //   marzReference (deposits + withdrawals) -- /deposit/callback and
    //     /withdraw/callback locate the row by reference with .limit(1). A
    //     duplicate reference means a webhook can resolve to the WRONG money
    //     row. The live provider re-check limits the blast radius; it does not
    //     make picking the wrong row safe.
    //   publicId -- the account number members read out to support.
    //   codeLower (promoCodes) -- one gift code redeemable as two.
    //
    // All are declared PARTIAL, on purpose. The plain (non-unique) index of
    // the same shape stays defined above and these carry their own explicit
    // names, so Mongo treats them as separate index specifications and there
    // is no IndexOptionsConflict against indexes already built on live data.
    // The partial filter is also what makes them correct rather than merely
    // strict: a unique index over a field that is ABSENT on many documents
    // would treat every one of those as the same null and reject the second
    // one. Manual deposits legitimately have no marzReference; accounts
    // predating the field have no publicId. Restricting each index to
    // documents where the field is actually a string means it constrains only
    // the rows it is meant to constrain.
    ['users', { referralCodeLower: 1 }, { unique: true, name: 'referralCodeLower_unique',
      partialFilterExpression: { referralCodeLower: { $type: 'string' } } }],
    ['users', { publicId: 1 }, { unique: true, name: 'publicId_unique',
      partialFilterExpression: { publicId: { $type: 'string' } } }],
    ['promoCodes', { codeLower: 1 }, { unique: true, name: 'codeLower_unique',
      partialFilterExpression: { codeLower: { $type: 'string' } } }],
    ['pendingDeposits', { marzReference: 1 }, { unique: true, name: 'marzReference_unique',
  ...
  const BATCH = 5;
  let failed = 0;
  // A performance index that fails to build costs speed. A UNIQUE index that
  // fails to build costs a guarantee -- and it fails for exactly one
  // interesting reason: the collection already contains the duplicates the
  // index was added to prevent. That is a finding, not a warning to scroll
  // past, so it is reported separately and repeated at the end where it can
  // still be seen after 30 other lines of startup output.
  const uniqueFailures = [];
  for (let i = 0; i < specs.length; i += BATCH) {
    const batch = specs.slice(i, i + BATCH);
    const results = await Promise.allSettled(
      batch.map(([col, keys, opts]) => _mdb.collection(col).createIndex(keys, opts || {})));
    results.forEach((r, j) => {
      if (r.status === 'rejected') {
        failed++;
        const [col, keys, opts] = batch[j];
        const msg = r.reason && r.reason.message;
        if (opts && opts.unique) {
          uniqueFailures.push(`${col} ${JSON.stringify(keys)}: ${msg}`);
          console.error(`UNIQUE index NOT built (${col} ${JSON.stringify(keys)}):`, msg);
        } else {
          console.warn(`Index build failed (${col} ${JSON.stringify(keys)}):`, msg);
        }
      }
    });
  }
  ...
  if (uniqueFailures.length) {
    console.error(
      '*** UNIQUENESS NOT ENFORCED on ' + uniqueFailures.length + ' index(es). The most ' +
      'likely cause is duplicate values ALREADY in the collection -- Mongo will not ' +
      'build a unique index over data that already violates it. Find and merge the ' +
      'duplicates, then redeploy to build the index:\n  ' + uniqueFailures.join('\n  '));
  }
}
