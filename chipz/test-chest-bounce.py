...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        info = await page.evaluate("""()=>{const c=document.querySelector('.chest-float:not(.turntable-float)');
          if(!c) return {missing:true};
          const cs=getComputedStyle(c);
          return {anim:cs.animationName, dur:cs.animationDuration, play:cs.animationPlayState,
                  transform:cs.transform, cls:c.className,
                  parentTransform:getComputedStyle(c.parentElement).transform,
                  parentClass:c.parentElement.className};}""")
        for k,v in info.items(): print("  %-16s %s" % (k,v))
        # sample the rendered position over time -- the only real proof it moves
        ys=[]
        for _ in range(8):
            ys.append(await page.evaluate("()=>document.querySelector('.chest-float:not(.turntable-float)').getBoundingClientRect().y"))
            await page.wait_for_timeout(220)
        print("  y samples:", [round(y,1) for y in ys])
        ck(max(ys)-min(ys) > 2, "chest actually moves on screen (range %.1fpx)" % (max(ys)-min(ys)))
