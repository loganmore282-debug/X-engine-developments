...
async def main():
    async with async_playwright() as pw:
        ...
        await page.evaluate("openDepositSheet()"); await page.wait_for_timeout(1200)
        m = await page.evaluate("""()=>{const h=s=>{const e=document.querySelector(s);return e?+e.getBoundingClientRect().height.toFixed(1):null;};
          const payAlign=(()=>{const p=document.querySelector('.pay-row span');if(!p)return null;
            const r=p.getBoundingClientRect(), row=p.closest('.pay-row').getBoundingClientRect();
            return {leftGap:+(r.left-row.left).toFixed(0), rightGap:+(row.right-r.right).toFixed(0)};})();
          return {depAmt:h('.dep-amt'), depPhone:h('.dep-phone'), payRow:h('.pay-row'),
        ...
        for fn,label,sels in [("openDepositSheet()","Deposit",['.dep-amt','.dep-phone']),
                              ...
                              ("openChangeLoginPasswordSheet()","Login password",['.pw-field']),
                              ("openChangeTradePasswordSheet()","Trade password",['.pw-field'])]:
            await page.evaluate("closeSheet()"); await page.wait_for_timeout(300)
            await page.evaluate(fn); await page.wait_for_timeout(800)
            hs = await page.evaluate("(sels)=>sels.flatMap(s=>[...document.querySelectorAll(s)].map(e=>s+':'+e.getBoundingClientRect().height.toFixed(0)))", sels)
            print("  %-15s %s" % (label, hs))
        await page.evaluate("closeSheet()"); await page.wait_for_timeout(300)
        await page.evaluate("openDepositSheet()"); await page.wait_for_timeout(900)
        await page.screenshot(path=f"{OUT}/deposit-after.png",full_page=True)
        await b.close()
