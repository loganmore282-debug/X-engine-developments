...
PATTERNS = [
    ...
    # Round 179b: maintenance mode and the opening countdown moved from
    # backend-wide to per-region (owner: "some settings affect whole
    # countries why?, see maintenance mode, countdown, please make sure that
    # everything is on its own"), so they moved out of this sentence's
    # "apply to every country" clause and into its per-country one, and
    # "first four" became "first two" (only the app name and the domain
    # allowlist are still edited on the founding country alone).
    ['Rates, limits, bonuses, payment methods, the announcement dialog, the cash-out window, maintenance mode and the opening countdown are saved for {0} — the country in the switch at the top. The app name, the domain allowlist and every uploaded image apply to every country, and the first two are only editable on {1}.',
     'Emiwendo, ebikomo, ebirabo, engeri z\'okusasula, akabokisi k\'obubaka, ekiseera ky\'okuggyamu, embeera y\'okuddaabiriza n\'okubala okw\'okuggulawo bitereka ku {0} — eggwanga eriri mu kakyusa waggulu. Erinnya lya app, olukalala lwa domain ezikkirizibwa n\'ebifaananyi byonna ebiteekeddwa bikola ku mawanga gonna, era ebisooka bibiri bikyusibwa ku {1} yokka.',
     'Viwango, mipaka, bonasi, njia za malipo, kisanduku cha tangazo, dirisha la utoaji, hali ya matengenezo na hesabu ya kufungua huhifadhiwa kwa {0} — nchi iliyo kwenye kibadilishi hapo juu. Jina la programu, orodha ya vikoa vinavyoruhusiwa na kila picha iliyopakiwa hutumika kwa kila nchi, na mbili za kwanza zinaweza kuhaririwa kwenye {1} pekee.',
     'Les taux, les limites, les bonus, les modes de paiement, la boîte d\'annonce, la fenêtre de retrait, le mode maintenance et le compte à rebours d\'ouverture sont enregistrés pour {0} — le pays du sélecteur en haut. Le nom de l\'application, la liste des domaines autorisés et chaque image téléversée s\'appliquent à tous les pays, et les deux premiers ne sont modifiables que sur {1}.',
     'Ibipimo, imipaka, ibihembo, uburyo bwo kwishyura, akazu k\'itangazo, idirishya ryo kubikuza, uburyo bwo gusana na kubara manuka byo gufungura bibikwa kuri {0} — igihugu kiri mu guhitamo haruguru. Izina rya porogaramu, urutonde rw\'amazina yemewe n\'ishusho yose yashyizweho bireba ibihugu byose, kandi ibya mbere bibiri bihinduka gusa kuri {1}.',
     'Emiihendo, ebikomo, ebiconco, emiringo y\'okushashura, akabokisi k\'obutumwa, obwire bw\'okwihamu, omuringo gw\'okugarura n\'okubara okw\'okwigura nibibikwa aha {0} — eihanga eriri omu kacwamu haiguru. Eiziina rya puroguramu, orutonde rwa domain ezirikwikirizibwa n\'ekishushani kyoona ekitairweho nibikora aha mahanga goona, kandi ebya mbere bibiri nibihindurwa aha {1} kwonka.'],
