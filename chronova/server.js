...
// Same Task Center, a second ladder — this one on the total money a user's
// LEVEL 1 referrals have deposited (never L2/L3), same manual-claim pattern.
// `target` is an amount, not a count.
...
// Sum of totalDeposited across this user's LEVEL 1 referrals only — recomputed
// live on every claim/stats read, same as activeL1Count, so this can never be
// forged from the client either.
...
function startCrons() {
  ...
  // Background payment settlement — fast, so a paid deposit lands within
  // moments even if the user closed the app and the callback never arrived
  ...
  // REVERTED from 2s (2026-07-30): suspected of hammering MarzPay's API
  // hard enough to trigger their own rate-limiting/"temporarily busy"
  // responses — one status-check call per stuck item, every single tick.
  // That would explain the busy errors hitting deposit COLLECTION too if
  ...
  // nobody the wiser. Kept on an automatic background cadence (was 2s,
  // reverted to 30s for the same rate-limit suspicion above).
  setInterval(pollPendingWithdrawalStatus, 30 * 1000);
  setTimeout(pollPendingWithdrawalStatus, 15 * 1000);
