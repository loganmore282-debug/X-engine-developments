...
(async () => {
  ...
  console.log('\n── 1. A flood against a PUBLIC, unauthenticated endpoint with no specific limiter of its own gets cut off by the global floor');
  ...
  console.log('\n── 2. /health is exempt from the global limiter — uptime monitoring must never be the thing that gets throttled');
  const healthResults = [];
